...
static int econvert(const char *str, int base, long *pLong, ulong *pULong, const char *errorMsg) {
	char *str1;
	long lResult;
	ulong uResult;
	...
	return emsgf(-1, errorMsg, str);
	}

// Convert a numeric literal to a long integer via econvert().
int estrtol(const char *str, int base, long *pResult) {

	return econvert(str, base, pResult, NULL, "Invalid signed number '%s'");
	}

// Convert a numeric literal to an unsigned long integer via econvert().
int estrtoul(const char *str, int base, ulong *pResult) {

	return econvert(str, base, NULL, pResult, "Invalid unsigned number '%s'");
	}
